...
const OccupiedClearButton = ({currentFocusedComponent, positionMap, setPositionMap }) => {
  ...
  return info ? (
    <div>
      <button className="occupied-button" onClick={clickHandler}>
        Occupied/Clear
      </button>
    </div>
  ) : (
    <button className="occupied-button">
    Occupied/Clear
  </button>
  )
}

export default OccupiedClearButton